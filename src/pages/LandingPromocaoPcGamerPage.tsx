...
import { BadgeCheck, CheckCircle2, Cpu, MapPin, MessageCircle, Phone, ShieldCheck, Timer, Truck, Wrench } from "lucide-react";

export default function LandingPromocaoPcGamerPage() {
  const whatsappNumber = "5519987510267";
  const whatsappLink = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(
    "Olá! Quero promoção de PC Gamer. Vou informar orçamento, jogos e se já tenho peças.",
  )}`;

  const title =
    "Promoção PC Gamer | Montagem e Upgrade com Suporte | Balão da Informática Campinas";
  const description =
    "Promoções de PC Gamer em Campinas: montagem sob medida, upgrade e testes. Indicação por orçamento/jogos, retirada no Cambuí ou envio expresso.";
  const keywords =
    "pc gamer promocao campinas, montar pc gamer campinas, upgrade pc gamer campinas, rtx pc gamer campinas, ryzen pc gamer campinas, intel pc gamer campinas, custo beneficio pc gamer, cable management campinas, testes stress pc campinas, windows instalado pc gamer, cambui av anchieta 789, campinas valinhos vinhedo paulinia sumare hortolandia indaiatuba";
  const url = "https://www.balao.info/lp/promocao-pc-gamer";

  const { products } = useProducts();
  const { config } = useLandingPageConfig("promocao-pc-gamer", {
    pageKey: "promocao-pc-gamer",
    gridQuery: "pc gamer",
    fallbackQueries: ["setup gamer", "rtx", "ryzen", "intel"],
  });

  const services = [
    "Montagem completa e organização de cabos",
    "Instalação do Windows e drivers",
    "Atualização de BIOS (quando aplicável)",
    "Testes de stress e estabilidade",
    "Ajuste de airflow e fan curve",
    "Upgrade de SSD/RAM/GPU/Fonte",
  ...
  return (
    <Layout>
      ...
      <BreadcrumbSchema items={[{ name: "Início", url: "https://www.balao.info" }, { name: "Promoção PC Gamer", url }]} />
      ...
      <div className="bg-white min-h-screen font-sans">
        ...
        >
          ...
          <span className="font-bold hidden md:inline">Orçar montagem</span>
        </a>
          ...
          <div className="container-balao relative z-10 grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl md:text-6xl font-black leading-tight">
                PC Gamer com
                <span className="text-yellow-400"> promoção e montagem</span>
              </h1>
              <p className="text-lg md:text-xl text-zinc-200 leading-relaxed">
                Indicamos peças por orçamento e objetivo, montamos e testamos. Retire hoje ou receba com envio rápido.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <p className="text-sm text-zinc-300">Produtos oficiais e suporte.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Wrench className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Montagem</p>
                  <p className="text-sm text-zinc-300">Instalação e testes.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Cpu className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Performance</p>
                  <p className="text-sm text-zinc-300">Mais FPS com estabilidade.</p>
                </div>
              </div>
            </div>

            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-2xl text-zinc-900 border border-zinc-200">
              <h2 className="text-2xl font-black mb-2">Informações para orçamento</h2>
              <p className="text-zinc-600 mb-6">Orçamento, jogos/uso e se já possui alguma peça.</p>
              <div className="space-y-3">
                {[
                  "Qual orçamento total? (ex: até R$ 2000, R$ 3500, R$ 5000)",
                  "Quais jogos/uso? (ex: CS2, AAA, edição, streaming)",
                  "Já possui peças? (liste modelo de fonte, GPU, SSD/RAM)",
                  "Preferências de estética/airflow/ruído?",
                ].map((t) => (
        ...
        <section className="bg-white py-16 border-b">
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <h2 className="text-3xl font-bold text-zinc-900">Serviços e vantagens</h2>
                <p className="text-zinc-600 mt-2">Montagem, upgrade e suporte.</p>
              </div>
              <Link to="/montagem-setup-gamer" className="text-[#E30613] font-semibold hover:underline">
                Ver montagem de setup
              </Link>
            </div>
        ...
        <section className="bg-zinc-50 py-16 border-b">
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <h2 className="text-3xl font-bold text-zinc-900">Peças em promoção</h2>
