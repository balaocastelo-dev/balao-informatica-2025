...
import { BadgeCheck, CheckCircle2, Cpu, MapPin, MessageCircle, Phone, ShieldCheck, Star, Timer, Truck, Zap } from "lucide-react";

export default function LandingPlacaDeVideoPromocaoPage() {
  const whatsappNumber = "5519987510267";
  const whatsappLink = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(
    "Olá! Quero promoção de placa de vídeo. Vou informar orçamento, jogos e resolução.",
  )}`;

  const title =
    "Placa de Vídeo em Promoção | RTX, GeForce, Radeon | Balão da Informática Campinas";
  const description =
    "Promoções de placa de vídeo em Campinas: RTX, GeForce e Radeon. Indicação por orçamento e objetivo, retirada imediata no Cambuí ou envio rápido.";
  const keywords =
    "placa de video promocao campinas, rtx promocao, geforce rtx campinas, radeon rx campinas, gpu barato campinas, custo beneficio gpu, upgrade pc gamer campinas, fps alto campinas, montagem pc gamer campinas, cambui av anchieta 789, campinas valinhos vinhedo paulinia sumare hortolandia indaiatuba";
  const url = "https://www.balao.info/lp/placa-de-video-promocao";

  const { products } = useProducts();
  const { config } = useLandingPageConfig("placa-de-video-promocao", {
    pageKey: "placa-de-video-promocao",
    gridQuery: "placa de video",
    fallbackQueries: ["rtx", "geforce", "radeon", "gpu", "placa de vídeo"],
  });

  const highlights = [
    "Indicação por orçamento e jogos",
    "Pronta entrega e envio expresso",
    "Garantia oficial e nota fiscal",
    "Suporte para instalação e drivers",
    "Troca de pasta térmica premium (consultar)",
    "Montagem e teste em bancada",
    "Combos com fonte/SSD/memória",
  ...
  return (
    <Layout>
      ...
      <BreadcrumbSchema items={[{ name: "Início", url: "https://www.balao.info" }, { name: "Placa de Vídeo Promoção", url }]} />
      ...
      <div className="bg-white min-h-screen font-sans">
        <div className="bg-yellow-400 text-zinc-900 font-black text-center py-3 px-4 flex items-center justify-center gap-2 animate-in slide-in-from-top duration-700">
          ...
          <span className="text-sm md:text-base uppercase tracking-wide">PROMOÇÕES EM CAMPINAS • ESCOLHA A GPU IDEAL PARA SEU USO</span>
        </div>
          ...
          <div className="container-balao relative z-10 grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl md:text-6xl font-black leading-tight">
                Placa de vídeo com
                <span className="text-yellow-400"> promoção e custo‑benefício</span>
              </h1>
              <p className="text-lg md:text-xl text-zinc-200 leading-relaxed">
                RTX, GeForce e Radeon. Indicação por orçamento, jogos e resolução. Retire hoje no Cambuí ou receba com envio expresso.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <p className="text-sm text-zinc-300">Produtos oficiais e testados.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Truck className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Retirada/envio</p>
                  <p className="text-sm text-zinc-300">Cambuí ou entrega rápida.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Zap className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Indicação</p>
                  <p className="text-sm text-zinc-300">Escolha certa para seu uso.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-2xl text-zinc-900 border border-zinc-200">
              ...
              <p className="text-zinc-600 mb-6">Orçamento, jogos e resolução para indicar a melhor GPU.</p>
              <div className="space-y-3">
                {[
                  "Qual orçamento? (ex: até R$ 900, R$ 1500, R$ 2500)",
                  "Quais jogos? (ex: CS2, Fortnite, GTA, simuladores)",
                  "Qual resolução/monitor? (ex: Full HD, 144Hz)",
                  "Vai streamar/gravar? precisa de encoding?",
                ].map((t) => (
        ...
        <section className="bg-white py-16 border-b">
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <h2 className="text-3xl font-bold text-zinc-900">Por que comprar aqui</h2>
                <p className="text-zinc-600 mt-2">Lista rápida de benefícios.</p>
              </div>
        ...
        <section className="bg-zinc-50 py-16 border-b">
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <h2 className="text-3xl font-bold text-zinc-900">GPUs em destaque</h2>
                <p className="text-zinc-600 mt-2">Grid filtrado por termos de GPU.</p>
              </div>
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
