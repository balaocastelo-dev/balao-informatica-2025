...
import { BadgeCheck, CheckCircle2, MapPin, MessageCircle, Network, Phone, Server, ShieldCheck, Timer, Wrench, Wifi } from "lucide-react";

export default function LandingVisitaTecnicaPage() {
  const whatsappNumber = "5519987510267";
  const whatsappLink = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(
    "Olá! Quero agendar visita técnica. Vou informar endereço, problema e janela de horário.",
  )}`;

  const title = "Visita Técnica em Campinas | Suporte de TI e Redes | Balão da Informática";
  const description =
    "Visita técnica em Campinas: suporte de TI, redes Wi‑Fi, roteadores, impressoras, PCs e servidores. Agendamento rápido com janela de horário.";
  const keywords =
    "visita tecnica campinas, suporte ti campinas, rede wifi campinas, roteador campinas, computador empresa campinas, impressora empresa campinas, servidor campinas, cabeamento campinas, configuração rede campinas, atendimento cambui campinas, av anchieta 789 campinas, ti empresas campinas, valinhos vinhedo paulinia sumare hortolandia indaiatuba";
  const url = "https://www.balao.info/lp/visita-tecnica";

  const { products } = useProducts();
  const { config } = useLandingPageConfig("visita-tecnica", {
    pageKey: "visita-tecnica",
    gridQuery: "",
    fallbackQueries: ["suporte", "ti", "network", "wifi", "roteador"],
  });

  const services = [
    "Configuração de rede Wi‑Fi e roteadores",
    "Ampliação de cobertura e estabilidade",
    "Cabeamento e organização",
    "Impressoras e servidores",
    "Computadores e notebooks",
    "Backup e políticas básicas",
    "Consultoria e boas práticas",
    "Atendimento empresas e residências",
  ];
  ...
  return (
    <Layout>
      ...
      <BreadcrumbSchema items={[{ name: "Início", url: "https://www.balao.info" }, { name: "Visita Técnica", url }]} />
      ...
      <div className="bg-white min-h-screen font-sans">
        <div className="bg-yellow-400 text-zinc-900 font-black text-center py-3 px-4 flex items-center justify-center gap-2 animate-in slide-in-from-top duration-700">
          ...
          <span className="text-sm md:text-base uppercase tracking-wide">AGENDAMENTO RÁPIDO • JANELA DE HORÁRIO E TRANSPARÊNCIA</span>
        </div>
        ...
        >
          ...
          <span className="font-bold hidden md:inline">Agendar visita</span>
        </a>
          ...
          <div className="container-balao relative z-10 grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl md:text-6xl font-black leading-tight">
                Visita técnica com
                <span className="text-yellow-400"> suporte de TI</span>
              </h1>
              <p className="text-lg md:text-xl text-zinc-200 leading-relaxed">
                Redes Wi‑Fi, roteadores, impressoras, PCs e servidores. Atendimento em empresas e residências de Campinas e região.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <p className="text-sm text-zinc-300">Janela de horário.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Wifi className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Redes</p>
                  <p className="text-sm text-zinc-300">Wi‑Fi e roteadores.</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <Server className="w-6 h-6 text-yellow-400 mb-2" />
                  <p className="font-bold">Infra</p>
                  <p className="text-sm text-zinc-300">Impressoras e servidores.</p>
                </div>
              </div>
            </div>

            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-2xl text-zinc-900 border border-zinc-200">
              <h2 className="text-2xl font-black mb-2">Para agendar</h2>
              <p className="text-zinc-600 mb-6">Endereço, problema e janela de horário desejada.</p>
              <div className="space-y-3">
                {[
                  "Qual endereço e tipo de local? (empresa/residência)",
                  "Qual problema? (rede, impressora, PCs/servidor, organização)",
                  "Disponibilidade: datas/horários preferidos",
                  "Necessita contrato/manutenção recorrente?",
                ].map((t) => (
        ...
        <section className="bg-white py-16 border-b">
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <h2 className="text-3xl font-bold text-zinc-900">Serviços de TI</h2>
                <p className="text-zinc-600 mt-2">Wi‑Fi, roteadores, impressoras, PCs e servidores.</p>
              </div>
              <Link to="/criacao-de-site-e-servicos-ti" className="text-[#E30613] font-semibold hover:underline">
                Ver serviços de TI
              </Link>
            </div>
          ...
          <div className="container-balao">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-zinc-600 mt-2">Grid filtrado por termos de TI/rede.</p>
              </div>
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
