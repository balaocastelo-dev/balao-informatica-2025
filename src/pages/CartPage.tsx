...
const CartPage = () => {
  ...
  const { appliedCoupon, discountAmount, status: couponStatus, isApplying, applyCoupon, removeCoupon, computeTotal } = useCoupon();
                ...
                <CardContent>
                  ...
                  <div className="border-t pt-4 space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          disabled={isApplying}
                          onClick={async () => {
                            const ok = await applyCoupon(couponInput, total);
                            if (!ok) {
                              toast({ title: 'Cupom inválido, expirado ou indisponível', variant: 'destructive' });
                            } else {
                    ...
                    <div className="flex justify-between text-lg font-bold pt-2 border-t">
                      <span>Total</span>
                      <span className="text-primary">{formatPrice(computeTotal(total))}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <div className="border-t border-border pt-3">
                  ...
                  {appliedCoupon && (
                    <p className="text-sm text-muted-foreground mt-1">
                      Desconto aplicado: -{formatPrice(discountAmount)}
                    </p>
                  )}
                  <p className="text-sm text-muted-foreground mt-1">
                    ou 12x de {formatPrice(computeTotal(total) / 12)} sem juros
                  </p>
                </div>
              </div>
